...
def get_dataset():
    ...
    fracture_dir = f"{path.path_root}/CompetitionData/PredictedFracture"
    data_split = pickle.load(open('../../DatasetConversion/data_split.pkl', 'rb'))
    ...
    for file in os.listdir(fracture_dir):
        ...
        if seg_bbox is None:
            print(f"        ----> No cervical vertebra found, skip")
            count -= 1
            continue
