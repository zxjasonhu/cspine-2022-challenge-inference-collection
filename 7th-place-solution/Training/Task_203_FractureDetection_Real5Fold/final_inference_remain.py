...
class FractureDetector:
    ...
    def read_DICOM_multi_thread(list_DICOM_dirs):
        ...
        for cur_thread in list_thread:
            threading.Thread.join(cur_thread)
...
if __name__ == '__main__':
    for fold in [0]:
        # --------------------------- Need Modification --------------------------------- #
        input_dir = f"{path.path_competition_data}/train_images"
        output_dir = f"{path.path_root}/CompetitionData/PredictedFractureV3"
        ...
        plan_C1_C7_segmentation = f'{path.path_root}/nnUnet/Models/nnUNet/3d_fullres/Task_103_VertebralLocation_GeneratePseudoLabel/nnUNetTrainerV2__nnUNetPlansv2.1/plans.pkl'
        ...
        splits_final = pickle.load(open(
            f'{path.path_root}/nnUnet/nnUNet_preprocessed/Task_203_FractureDetection_Real5Fold/splits_final.pkl',
            'rb'))
        list_DICOM_dirs =[]
        if fold == 0:  # Five fold CV do not use negative samples, so we need to fold 0 to inference remaininng cases
            for file in os.listdir(input_dir):
                case_id = file.split('.nii')[0]
                if case_id not in (list(splits_final[fold]['val']) + list(splits_final[fold]['train'])):
                    list_DICOM_dirs.append(f"{input_dir}/{file}")
        # -------------------------------------------------------------------------------- #
