...
class ClassificationTarget:
    def __init__(self, category):
        """
        Constructor for the ClassificationTarget class.

        Parameters:
        - category (int): The category ID that needs to be visualized.
        """
        self.category = category

    def __call__(self, model_output):
        """
        This method will be called with the model's output. It returns
        the output score for the given category.

        Parameters:
        - model_output (torch.Tensor): The output from the model.

        Returns:
        - torch.Tensor: The model's output for the category.
        """
...
class MDAIModel:
    def __init__(self):
        ...
        self.seg_model = load_model(configs["segmentor_csn_ir152"],
                                    os.path.join(root_path, "4th-place", "checkpoints",
                                                 "256_ResNet3dCSN2P1D_r50ir_1_dice")).to(
            self.device)
    ...
    def _get_image_cubes_classification_explain(self, data, mask_cube: np.ndarray) -> torch.Tensor:
    ...
    def explain_gradcam(self, data):
        ...
        sample = self._get_image_cubes_classification_explain(data, self._predict_seg_mask(data))
        input_tensor = sample['image']
        image_cube = sample['image_cube']
        input_tensor_info = sample['info']

        # print(f"image_cube shape", image_cube.shape)
        ...
        for model in models:
            ...
            with GradCAM3D(model=model, target_layers=target_layers, use_cuda=torch.cuda.is_available()) as cam:
                ...
                if full_mask is None:
                    full_mask = interpolate_cam_on_voxel(image_cube, grayscale_cam[0], input_tensor_info)
                else:
                    full_mask += interpolate_cam_on_voxel(image_cube, grayscale_cam[0], input_tensor_info)

        if full_mask is not None:
            full_mask = full_mask / len(models)
            # display(image_cube, full_mask)
            return full_mask#, input_tensor, grayscale_cam
