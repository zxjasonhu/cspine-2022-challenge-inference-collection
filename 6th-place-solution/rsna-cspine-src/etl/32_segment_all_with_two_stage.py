...
def segment_two_stage(X, input_size1, input_size2, model_list1, model_list2, plot=False):
    ...
    for level in range(pseg.shape[0]):
        coords = np.vstack(np.where(pseg[level] >= 0.5))
        if coords.shape[1] == 0:
    ...
    return coords_dict,\
           ...
           F.interpolate(torch.from_numpy(pseg).unsqueeze(0), size=(Z, H, W), mode="nearest").squeeze(0)
...
SAVE_DIR1 = "../../data/train-individual-vertebrae"
...
checkpoint_paths1 = np.sort(glob.glob("../../experiments/seg100/sbn/fold*/checkpoints/best.ckpt"))
checkpoint_paths2 = np.sort(glob.glob("../../experiments/seg101/sbn/fold*/checkpoints/best.ckpt"))
...
studies = list(np.sort(df.StudyInstanceUID.unique()))[428:1000]
...
for study_id, study_df in tqdm(df.groupby("StudyInstanceUID"), total=len(df.StudyInstanceUID.unique())):
    ...
    coords_dict, crop_cervical_coords, pseg = segment_two_stage(rescale(torch.from_numpy(study_stack).float()).cuda(),
    ...
    #plot_volume(cropped_stack, 6, 4)
    for level, coords in coords_dict.items():
        if isinstance(coords, type(None)):
            print(f"{study_id} C{level+1} FAILED !")
            np.save(osp.join(SAVE_DIR1, f"{study_id}_C{level + 1}.npy"), np.zeros((5, 5, 5)))
            np.save(osp.join(SAVE_DIR2, f"{study_id}_C{level + 1}.npy"), np.zeros((5, 5, 5)))
            continue
        z1, z2, h1, h2, w1, w2 = coords
        vertebra = cropped_stack[z1:z2+1, h1:h2+1, w1:w2+1]
        #plot_volume(vertebra, 6, 4, sagittal=False)
        masked_vertebra = vertebra.copy()
        masked_vertebra[pseg[level, z1:z2+1, h1:h2+1, w1:w2+1] < 0.5] = 0
        #plot_volume(masked_vertebra, 6, 4, sagittal=False)
        np.save(osp.join(SAVE_DIR1, f"{study_id}_C{level + 1}.npy"), vertebra)
        np.save(osp.join(SAVE_DIR2, f"{study_id}_C{level + 1}.npy"), masked_vertebra)
