...
IMSIZE = 192.
CONFIG = "../configs/seg/seg001.yaml"
SAVE_DIR = "../../data/train-numpy-vertebra-chunks-masked/"
create_dir(SAVE_DIR)
...
for study_id, study_df in tqdm(df.groupby("StudyInstanceUID"), total=len(df.StudyInstanceUID.unique())):
    ...
    p_up = torch.nn.functional.interpolate(p.unsqueeze(0), size=(z, h, w), mode="nearest").squeeze(0)
    for level in range(7):
        ...
        z1, z2, h1, h2, w1, w2 = p_level[0].min(), p_level[0].max(), p_level[1].min(), p_level[1].max(), \
                                 p_level[2].min(), p_level[2].max()
        chunk = orig_X[z1:z2+1, h1:h2+1, w1:w2+1]
        chunk[p_up[level][z1:z2+1, h1:h2+1, w1:w2+1].numpy() < adjusted_threshold] = 0
        np.save(osp.join(SAVE_DIR, f"{study_id}_C{level+1}.npy"), chunk)