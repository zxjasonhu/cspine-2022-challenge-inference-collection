...
SAVE_DIR = "../../data/train-numpy-vertebra-chunks-with-seg/"
create_dir(SAVE_DIR)
...
chunk_df["level"] = chunk_df.study_with_level.apply(lambda x: x.split("_")[1])
for i in range(1, 8):
    chunk_df[f"C{i}_frac"] = 0
    chunk_df.loc[(chunk_df["level"] == f"C{i}") & (chunk_df["fracture"] == 1), f"C{i}_frac"] = 1
    print(chunk_df[f"C{i}_frac"].value_counts())

chunk_df.to_csv("../../data/train_vertebra_chunks_with_seg_kfold.csv", index=False)